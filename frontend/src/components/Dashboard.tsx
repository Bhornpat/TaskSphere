		...
		<div className="p-6">
			...
			<div className='px-6 flex justify-center items-center'>
				<button
					...
					className="my-6 px-6 py-3 rounded-2xl bg-gradient-to-br from-pink-600 via-red-500 to-yellow-400 text-white text-xl shadow-md font-bold hover:brightness-110 transition duration-200"
				>
					Add Task Here
				</button>
			</div>
			<div className="space-y-6 px-6">
				{tasks.map(task => (
					<div key={task.id}
						className={`p-5 rounded-lg border flex justify-between items-center ${task.status === 'done' ? 'bg-green-100' : 'bg-yellow-50'}`}
					>
						<div>
							{/*title*/}
							<p className="font-bold text-xl text-gray-700 break-all leading-relaxed">{task.title}</p>
							{/*description*/}
							<div className="text-md text-gray-700 leading-relaxed font-mono break-words whitespace-normal"
							>
								{task.description.length > 58 ? (
									expandedTaskId[task.id] ? (
										...
										<>
											<p className="break-words whitespace-normal">{task.description.slice(0, 105) + '...'} </p>
											<button
							...
							<div className='flex flex-wrap gap-2 mt-3'>
								<button
									...
									className="px-4 py-2 rounded-full bg-green-600 text-white text-xs shadow-md font-bold hover:brightness-110 transition duration-200"
								>
									Edit
								</button>
						...
						<div className="shrink-0 w-[100px] text-right">
							<button
								...
								className={`hidden md:inline-block text-sm px-4 py-2 rounded-full hover:scale-105 transition duration-300 shadow-md font-bold cursor-pointer ${task.status === 'done' ? 'bg-green-500 text-white' : 'bg-yellow-400 text-black'
									}`}
							>
						...
						<div className="relative  right-2 bg-[#fffde7] p-0 rounded-l z-50 bg-transparent">
							<button
								onClick={async () => {
									const token = localStorage.getItem('token');
									if (!token) return;
							...
							>
								<span className={`inline-block md:hidden text-xl rounded-full px-3 py-2
